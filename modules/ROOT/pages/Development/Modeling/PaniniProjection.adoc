...
== In-Game Appearance Screenshots

You can tell a material is not set up properly if its appearance changes noticeably
when the user adjusts their FOV settings.
...
The left side shows has the projection enabled, while the right side has it disabled.
...
It can be enabled from material instances via their material parameters, usually called something like "UsePaniniProjection".
