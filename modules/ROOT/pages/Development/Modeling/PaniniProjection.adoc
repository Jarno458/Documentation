...
a post-processing shader called a **Panini Projection** is applied to the world and the player's first person models.
...
. Add a `StaticSwitch` to toggle Panini on/off during testing, and set its Value to true.
. Feed the result of the Panini Projection into the *Static Switch True* input.
. Plug a Constant3Vector node of `0,0,0` into the *Static Switch False* input.
. Plug the result into your material's World Position Offset output.
. Do whatever else you were planning to do with the other material outputs.
